...
/**
 * 
 * Local manager of the current BungeeCord instance, without any network
 * connection. The purpose of this class is to be able to recover players
 * connected to the current BungeeCord, as well as their data.
 * 
 * @author xMalware
 *
 */
public class BungeeLocalManager {
	...
	public BadPlayer getPlayer(UUID uuid) {
		...
		Optional<BadPlayer> optional = BadPlayer.getPlayers().parallelStream()
				.filter(player -> player.getUniqueId().equals(uuid)).findFirst();
	 ...
	 * 
	 * @param current
	 *            player name you're looking for
	...
	public boolean isOnline(UUID uuid) {
		// We count the number of local players online and if the count is more than 0,
		// then he's online
