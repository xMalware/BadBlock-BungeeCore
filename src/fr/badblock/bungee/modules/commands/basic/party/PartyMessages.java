...
public class PartyMessages {
	...
	@Getter
	@Setter
	private String prefix = "bungee.commands.party.";
	...
	public void sendInviteYouHaveBeenInvited(BadPlayer otherPlayer, String byPlayer) {
		...
		String decline_hover = otherPlayer.getTranslatedMessage(prefix + "invite.receive.decline_hover", null,
				byPlayer);
		...
		McJson json = new McJsonFactory(intro).finaliseComponent().initNewComponent(accept).setHoverText(accept_hover)
				.setClickCommand("/party accept " + byPlayer).finaliseComponent().initNewComponent(decline)
				.setHoverText(decline_hover).setClickCommand("/party leave").finaliseComponent().build();
	...
	public void sendList(BadPlayer badPlayer) {
		...
		String with_only_his_friends = badPlayer.getTranslatedMessage(prefix + "toggle.set.with_only_his_friends",
				null);
		// Get 'with_only_his_friends_hover' message
		String with_only_his_friends_hover = badPlayer
				.getTranslatedMessage(prefix + "toggle.set.with_only_his_friends_hover", null);
		...
		// Create McJson message
		McJson json = new McJsonFactory(intro).finaliseComponent().initNewComponent(with_everyone)
				.setHoverText(with_everyone_hover).setClickCommand("/party toggle with_everyone").finaliseComponent()
				.initNewComponent(with_only_his_friends).setHoverText(with_only_his_friends_hover)
				.setClickCommand("/party toggle with_only_his_friends").finaliseComponent()
				.initNewComponent(with_nobody).setHoverText(with_nobody_hover)
				.setClickCommand("/party toggle with_nobody").build();
	...
	public void sendPartyListAccepted(BadPlayer badPlayer, boolean canRemove, String playerName, boolean follow) {
		...
		// Create McJson message
		McJsonFactory jsonFactory = new McJsonFactory(state).setHoverText(state_hover).finaliseComponent()
				.initNewComponent(accepted).finaliseComponent().initNewComponent(mfollow).setHoverText(mfollow_hover)
				.finaliseComponent();
		...
		if (canRemove) {
			...
			// Set command
			String command = badPlayer.getName().equalsIgnoreCase(playerName) ? "/party leave"
					: "/party remove " + playerName;
			// Append to the message
			jsonFactory = jsonFactory.initNewComponent(spacer).finaliseComponent().initNewComponent(remove)
					.setHoverText(remove_hover).setClickCommand(command).finaliseComponent();
		}
	...
	public void sendPartyListAdmin(BadPlayer badPlayer, boolean canRemove, String playerName, boolean follow) {
		...
		// Get 'follow_hover' message
		String mfollow = badPlayer.getTranslatedMessage(prefix + "list." + (!follow ? "dont" : "") + "follow", null,
				playerName);
		// Get 'decline' message
		String mfollow_hover = badPlayer
				.getTranslatedMessage(prefix + "list." + (!follow ? "dont" : "") + "follow_hover", null, playerName);

		// Create McJson message
		McJsonFactory jsonFactory = new McJsonFactory(state).setHoverText(state_hover).finaliseComponent()
				.initNewComponent(admin).finaliseComponent().initNewComponent(mfollow).setHoverText(mfollow_hover)
				.finaliseComponent();
	...
	public void sendPartyListModo(BadPlayer badPlayer, boolean canRemove, String playerName, boolean follow) {
		...
		// Create McJson message
		McJsonFactory jsonFactory = new McJsonFactory(state).setHoverText(state_hover).finaliseComponent()
				.initNewComponent(modo).finaliseComponent().initNewComponent(mfollow).setHoverText(mfollow_hover)
				.finaliseComponent();
	...
	public void sendPartyListWaiting(BadPlayer badPlayer, boolean canRemove, String playerName) {
		...
		// Create McJson message
		McJsonFactory jsonFactory = new McJsonFactory(state).setHoverText(state_hover).finaliseComponent()
				.initNewComponent(waiting).finaliseComponent();
	...
	public void sendToggleWith(ProxiedPlayer proxiedPlayer, String type) {
		// Send username
		I19n.sendMessage(proxiedPlayer, prefix + "toggle.with", null,
				I19n.getMessage(proxiedPlayer, prefix + "toggle." + type, null));
	}
